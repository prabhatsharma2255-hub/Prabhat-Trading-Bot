...
class MoveDetector:
    ...
    def poll_funding_rate(self):
        ...
        try:
            url = "https://deltaexchange.io/v2/tickers"
    ...
    def poll_oi(self):
        ...
        try:
            url = "https://deltaexchange.io/v2/open_interest"
            response = requests.get(url, timeout=10)
    ...
    def get_market_intelligence(self) -> Dict:
        return {
            ...
            "velocity_1m": (self.price_history[-1] - self.price_history[-2]) / self.price_history[-2] * 100 if len(self.price_history) >= 2 else 0,
            "velocity_3m": (self.price_history[-1] - self.price_history[-4]) / self.price_history[-4] * 100 if len(self.price_history) >= 4 else 0,
        }
